...
# 1. Regress price on review_scores_rating. Plot the regression
# line and the actual training points, and find the out-of-sample
# RMSE. (Read below for more details if you need them.)
...
lm_price <- lm(price ~ review_scores_rating,
               data = listings_pr_by_rev$train)
...
# 2. Try to beat the out-of-sample performance of the
...
# 3. Install the quantreg package, and compare the behavior of
# the median regression fit (using the `rq()) function) to the
# least squares fit from `lm()` on the original listings data
# which includes all the price outliers. You can enter ?rq for
# info on the rq function.

# (More details: One easy way to compare the quantitative behavior
# is by plotting the two regression lines together. You can use
# the `gather_predictions()` function as we did in class.)
...
# between the two. One could use cross validation to choose this
...
# How do the new models do on out-of-sample RMSE?
...
# 1. Try to beat the out-of-sample performance for logistic

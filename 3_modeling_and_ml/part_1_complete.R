...
#' There are lots of ways to do this. We'll use the `caTools` package.
...
big_price_lm <- lm(big_formula, data = listingsBigTrain)	

#' What happens when we compare in-sample and
#' out-of-sample prediction performance?	
pred_test <- predict(big_price_lm, newdata = listingsBigTest)
OSR2_big <- 1 - sum((pred_test - listingsBigTest$price) ^ 2) /
  sum((mean(listingsBigTrain$price) - listingsBigTest$price) ^ 2)
summary(big_price_lm)$r.squared # In-sample
OSR2_big # Out-of-sample
...
x <- model.matrix(~ . - price + accommodates*room_type + 
                    ...
                    review_scores_rating*neighbourhood_cleansed + 
                    accommodates*review_scores_rating,
                  data = listingsBigTrain)
y <- as.vector(listingsBigTrain$price)
...
x_all <- model.matrix(~ . - price + accommodates*room_type +
                        property_type*neighbourhood_cleansed +
                        review_scores_rating*neighbourhood_cleansed +
                        accommodates*review_scores_rating,
                      data = listings_big)
...
listingsGLMTrain %>%	
  mutate(pred = predict(l.glm, data = listingsGLMTrain, type = "response")) %>%	
  ggplot(aes(x = price)) + 
...
library(ROCR)	
preds <- predict(l.glm, listingsGLMTest, type = "response")	
pred_obj <- prediction(preds, listingsGLMTest$amenity_Elevator_in_Building)
...
l.rpart <- rpart(amenity_Elevator_in_Building ~ price + 
                  neighbourhood_cleansed, data = listingsGLMTrain)	
summary(l.rpart)	

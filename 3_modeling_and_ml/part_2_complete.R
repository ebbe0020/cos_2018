...
# Rerun the CART model with the following code, and check the
# out-of-sample performance. Does it improve?  Why or why not?
set.seed(123)
spl <- sample.split(commentsTM$RATING_TEXT, SplitRatio = 0.7)
commentsTrain <- subset(commentsTM, spl == TRUE)
commentsTest <- subset(commentsTM, spl == FALSE)
...
# (Hint: To build the LASSO model, follow the instructions in
# part 1 of this module.)
# **Solution:**
# Split data into training and testing sets
# install.packages("caTools")
library(caTools)
...
str(listingsTM)
BagOfWords_LASSO_cv <-
  cv.glmnet(model.matrix(~ . - PRICE, data = listingsTrain),
            listingsTrain$PRICE)
BagOfWords_LASSO_cv

# Test Error
newx <- model.matrix(~ . - PRICE, data = listingsTest)
pred_test <- BagOfWords_LASSO_cv %>%
  predict.cv.glmnet(newx = newx) %>%
  as.vector()
test_rmse <- sqrt(mean((listingsTest$PRICE - pred_test) ^ 2))
test_rmse

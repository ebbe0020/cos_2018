...
# Note: I use base R instead of ggplot2 for the really
# simple first-blush plots	
...
#' You may argue that it's still a bit messy, but let's see what
#' we can do with a linear model. Since we care about prediction
#' accuracy, we'll reserve a portion of our data to be a test set.
#' There are lots of ways to do this. We'll use the `modelr` package,
#' which is part of the `tidyverse`.	
library(modelr)
#' This comes with tidyverse installation, but doesn't automatically load with the library(tidyverse) call.
listings_for_lm <- listings_for_lm %>% 	
  resample_partition(c(train = 0.7, test = 0.3))

#' The object `listings_for_lm` is now a list with two
#' elements: `train` and `test`.	
...
lm_price_by_acc <- lm(price ~ accommodates, data = listings_for_lm$train)
#' We'll talk more about the '~' notation soon	

#' Let's check out the lm_price_by_acc object:	
names(lm_price_by_acc)

#' The object `lm_price_by_acc` is a list of a bunch of relevant
#' information generated by the `lm()` function call. We can use
#' the `$` to view different elements, for example:
lm_price_by_acc$call

#' So, it stores the function call in one of the list elements.
#' But this isn't the most useful way to check out the model fit.
#' The function `summary()` is overloaded for many different objects
#' and often gives a useful snapshot of the model:	
...
#' so well here and we could refine it in a later modelling iteration.	
...
#' the prediction error. We'll look at two:	

#' *Root Mean-squared Error (RMSE): $\sqrt{\sum_{t=1}^n (\hat{y}_t - y_t)^2/n}$	

#' *Mean Absolute Error (MAE): $\sum_{t=1}^n |\hat{y}_t - y_t|/n$	

#' In both, $\hat{y}_t$ is the predicted value for test observation $t$,
#' $y_t$ is the actual value, and $n$ is the size of the test set.	

#' You could do this pretty easily by hand, and we'll do so later on,
#' but `modelr` has built-in functions to do this for you:	
rmse(lm_price_by_acc, listings_for_lm$test)
mae(lm_price_by_acc, listings_for_lm$test)	

#' These don't help much on their own, but they come in handy when
#' comparing different models, which we'll do next after a quick review.	
...
#' out-of-sample performance. In our case, we used the `modelr`
...
#' just a bit tricky, so I've written a function called `clean_amenities`
...
#' First, let's clean up the data by getting rid of missing values
#' and outliers. For categorical variables, we will remove all
#' categories with only a few observations. Finally, we'll
...
all_amenities <- as.data.frame(listings_big_lm$train) %>%
  select(starts_with("amenity")) %>% names()	
...
#' We won't look at the summary because there are so many
#' predictors. What happens when we compare in-sample and
...
#' We've got an overfitting problem here, meaning that the
#' training error is smaller than the test error. The model is too
#' powerful for the amount of data we have. Note that R recognizes
#' this by giving warnings about a "rank-deficient fit."	
...
#' Notice also that there's a way to specify lambda manually.
#' Since we haven't discussed choosing lambda yet, let's just
#' accept the default for now and see what we get.
x <- model.matrix(~ .-price + accommodates*room_type + 
                    property_type*neighbourhood_cleansed + 
                    review_scores_rating*neighbourhood_cleansed + 
                    accommodates*review_scores_rating,
                  data = as.data.frame(listings_big_lm$train))	
...
#' function for `glmnet` objects uses matrices, we can't use the
#' `rmse` function like we did before.	
x_all <- model.matrix(~ .-price + accommodates*room_type + property_type*neighbourhood_cleansed + review_scores_rating*neighbourhood_cleansed + accommodates*review_scores_rating,data=listings_big) 
# Matrix form for combined test and training data	
listings_big %>%	
  mutate(is_test = 1:nrow(listings_big) %in% listings_big_lm$test$idx,	
         pred = as.vector(predict.cv.glmnet(lasso_price_cv,
                                            newx = x_all))) %>%	
  group_by(is_test) %>%	
...
#' We'll also filter out price outliers.	
...
l.glm <- glm(amenity_Elevator_in_Building ~ price,
            family = "binomial", data = listings_glm$train)
summary(l.glm)	
...
as.data.frame(listings_glm$train) %>%	
  mutate(pred = predict(l.glm,data = listings_glm$train, 
                        type = "response")) %>%	
  ggplot(aes(x = price)) + 
...
#' different: a probability thateach listing has an elevator. This
#' gives us flexibility in the way wepredict. The most natural
...
library(ROCR)	
preds <- predict(l.glm,as.data.frame(listings_glm$test),
                 type = "response")	
test_elevators <-
  as.data.frame(listings_glm$test)$amenity_Elevator_in_Building	
pred_obj <- prediction(preds, test_elevators)
...
#' regression there are many, many extensions. For example, we could add
#' higher-order predictor terms via splines. We could also do LASSO
#' logistic regression if we wanted to use many predictors, using
#' the `glmnet` package.	
...
l.rpart <- rpart(amenity_Elevator_in_Building ~ price + 
                  neighbourhood_cleansed, data = listings_glm$train)	
summary(l.rpart)	

#' This is another case when the `summary()` function is less
#' helpful. We can, however, plot the resulting tree:	
library(rpart.plot)	
prp(l.rpart)	

#' The neighbourhood_cleansed variable names are a bit hard to read,
#' but other than that the model is clear and simple. While logistic
#' regression is a continuous, global method, classification trees
#' is a piecewise constant, local method. In the next section we'll
#' look at a much fancier classification tree and discuss
